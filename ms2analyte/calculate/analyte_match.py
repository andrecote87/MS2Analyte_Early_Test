...
def relative_intensity_match(spectrum1, spectrum2, condition):
    ...
    'condition' determines the logic to return TRUE
    The AND version is bidirectional (i.e. A must be at least 50% of B and vice versa to return TRUE)
    The OR version is unidirectional (i.e. either A must be 50% of B or vice versa to return TRUE)
    ...
    if len(spectrum1_matched_peaks) > 0 and len(spectrum2_matched_peaks) > 0:
        ...
        if condition == "AND":
            ...
            if spectrum1_match_intensity/spectrum1_intensity >= config.analyte_match_score \
                    or spectrum2_match_intensity/spectrum2_intensity >= config.analyte_match_score:
                return True
