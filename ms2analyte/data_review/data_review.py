...
def sample_analyte_review(file_path, input_type, sample_name, ms_type):
    """Print all available data for each sample analyte in a given sample. Requires full sample name including
    replicate suffix

    """
    with open(os.path.join(file_path, input_type, sample_name + "_" + ms_type + "_analytes.pickle"), "rb") as f:
...
def sample_analyte_dda_review(file_path, input_type, sample_name, ms_type):
    """Print DDA data for each sample analyte in a given sample. Requires full sample name including
    replicate suffix
    ...
    for analyte in sample_analytes:
        ...
        for peak in analyte.peak_list:
            if peak.dda_data:
                print("Mass peak: " + str(peak.average_mass))
                print(peak.dda_data)
...
def replicate_analyte_mass_spectra_review():
    # """Open each sample file and print MS data for each replicate analyte"""
    input_structure = data_import.input_data_structure()
    sample_type = "Samples"
    sample_list = data_import.name_extract(input_structure, sample_type)
    ...
    for sample_name in sample_list:
        with open(os.path.join(input_structure.output_directory,
            ...
            sample_spectra = pickle.load(pickle_file)

        for replicate_analyte in sample_spectra:
            print("Replicate analyte: " + str(replicate_analyte.replicate_analyte_id))
            print("MS1 spectrum: " + str(format_spectrum(replicate_analyte, "ms1")))
            print("MS2 spectrum: " + str(format_spectrum(replicate_analyte, "ms2")))


def format_spectrum(analyte, ms_type):
    """Extract MS peaks from MS object to list"""
    ms_list = []
    if ms_type == "ms1":
        peak_list = analyte.replicate_analyte_ms1_spectrum
    elif ms_type == "ms2":
        peak_list = analyte.replicate_analyte_ms2_spectrum
    else:
        return None

    if peak_list is not None:
        for peak in peak_list:
            ms_list.append([peak.average_mass, peak.average_intensity])
    else:
        return None
