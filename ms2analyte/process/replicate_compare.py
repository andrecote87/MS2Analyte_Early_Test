...
class ReplicateAnalyte:
    ...
    def __init__(self, replicate_analyte_id, analyte_list, max_peak_intensity_mass, max_intensity, replicate_analyte_rt,
                 replicate_analyte_ms1_spectrum, replicate_analyte_ms2_spectrum, sample_name, is_basketed):
...
def sample_consensus_analytes(input_structure, input_type, ms_type):
    ...
    for sample_name in sample_name_list:
        ...
        for i in range(1, input_structure.replicate_count + 1):
            with open((os.path.join(input_structure.output_directory, input_type, sample_name + "_R"
                                    + str(i) + "_" + ms_type + "_analytes.pickle")), "rb") as pickle_file:
        ...
        for index, replicate_analytes in enumerate(sample_replicate_data):
            input_data_replicate_annotate(input_structure,
                                          input_type,
                                          sample_name,
                                          index + 1,
                                          replicate_analytes,
        ...
        tableau.replicate_analyte_export(input_structure, input_type, sample_name, "ms1")
        if input_structure.ms2_exists and input_structure.ms2_type == "DIA":
            tableau.replicate_analyte_export(input_structure, input_type, sample_name, "ms2")

        replicate_analyte_objects(input_structure, input_type, sample_name)


def sample_consensus_analytes_no_replicates(input_structure, input_type, ms_type):
    ...
    for sample_name in sample_name_list:
        with open((os.path.join(input_structure.output_directory,
                                input_type,
                                sample_name + "_R1_" + ms_type + "_analytes.pickle")), "rb") as pickle_file:
            replicate_data = pickle.load(pickle_file)
        ...
        input_data_replicate_annotate(input_structure, input_type, sample_name, 1, sample_replicate_data, "ms1")
        # If there is ms2 data then add replicate analyte id column. replicate analyte id == analyte id because there
        # is only one replicate
        if input_structure.ms2_exists:
            if input_structure.ms2_type == "DIA":
                with open((os.path.join(input_structure.output_directory,
                                        input_type,
                                        sample_name + "_R1_ms2_dataframe.pickle")), "rb") as pickle_file:
                    ms2_data = pickle.load(pickle_file)
                ms2_data["replicate"] = 1
                ms2_data["replicate_analyte_id"] = ms2_data["analyte_id"]
                with open((os.path.join(input_structure.output_directory,
                                        input_type,
                                        sample_name + "_R1_ms2_dataframe.pickle")), "wb") as pickle_output:
                    pickle.dump(ms2_data, pickle_output)

        tableau.replicate_analyte_export(input_structure, input_type, sample_name, ms_type)
        replicate_analyte_objects(input_structure, input_type, sample_name)


def input_data_replicate_annotate(input_structure, input_type, sample_name, replicate_number, analyte_list, ms_type):
    ...
    with open((os.path.join(input_structure.output_directory, input_type, sample_name + "_R" + str(replicate_number)
              + "_" + ms_type + "_dataframe.pickle")), "rb") as pickle_file:
    ...
    with open((os.path.join(input_structure.output_directory, input_type, sample_name + "_R" + str(replicate_number)
              + "_" + ms_type + "_dataframe.pickle")), "wb") as pickle_file:
        pickle.dump(input_data, pickle_file)


def replicate_analyte_objects_old(input_structure, input_type, sample_name):
...
def replicate_analyte_objects(input_structure, input_type, sample_name):
    ...
    # Load dataframes for all replicates for a sample, and concatenate into one dataframe
    replicate_ms1_data = file_load.sample_dataframe_concat(input_structure, input_type, sample_name, "ms1")
    if input_structure.ms2_exists:
        if input_structure.ms2_type == "DIA":
            replicate_ms2_data = file_load.sample_dataframe_concat(input_structure, input_type, sample_name, "ms2")
    else:
        replicate_ms2_data = None

    sample_replicate_data = []

    # For each replicate analyte, find mass features from max intensity scan from each replicate, then determine
    # the relative activities (100%) for each replicate, and determine average values for each peak.
    for replicate_analyte_id, replicate_analyte_ms1_data in replicate_ms1_data.groupby("replicate_analyte_id"):
        # Process MS1 data
        replicate_ms1_mass_peak_data, \
        analyte_list, \
        replicate_analyte_rt, \
        max_intensity_mass, \
        max_intensity = extract_replicate_ms_data(input_structure, replicate_analyte_ms1_data, sample_name)

        # Process MS2 data
        if input_structure.ms2_exists:
            if input_structure.ms2_type == "DIA":
                replicate_analyte_ms2_data = replicate_ms2_data[replicate_ms2_data["replicate_analyte_id"] ==
                                                                replicate_analyte_id]
                replicate_ms2_mass_peak_data, \
                ms2_analyte_list, \
                ms2_replicate_analyte_rt, \
                ms2_max_intensity_mass, \
                ms2_max_intensity = extract_replicate_ms_data(input_structure, replicate_analyte_ms2_data, sample_name)
            else:
                replicate_ms2_mass_peak_data = None
        else:
            replicate_ms2_mass_peak_data = None

        sample_replicate_data.append(ReplicateAnalyte(replicate_analyte_id,
                                                      analyte_list,
                                                      max_intensity_mass,
                                                      max_intensity,
                                                      replicate_analyte_rt,
...
def analyte_update_replicate_analyte_data(input_structure, input_type, sample_names):
    ...
    for sample_name in sample_names:
        ...
        for i in range(1, input_structure.replicate_count + 1, 1):
            with open((os.path.join(input_structure.output_directory, input_type, sample_name + "_R"
                                    + str(i) + "_ms1_analytes.pickle")), "rb") as g:
...
def extract_max_intensity_spectra(replicate_analyte_data, sample_name):
    """Extract mass peaks from maximum intensity scan for each replicate"""
    ...
    for replicate, analyte_data in replicate_analyte_data.groupby("replicate"):
        max_scan = analyte_data[analyte_data["intensity"] == analyte_data["intensity"].max()].iloc[0]["scan"]
        max_intensity = analyte_data["intensity"].max()
        analyte_data["relative_intensity"] = analyte_data["intensity"] * (100/max_intensity)
        filtered_data = analyte_data[analyte_data["scan"] == max_scan]
        insert_data = filtered_data[["mz", "intensity", "relative_intensity"]].round({"mz": 4, "intensity": 0, "relative_intensity": 2}).values.tolist()
        replicate_max_mass_data.append(ReplicateMaxMassData(replicate, sample_name, max_scan, max_intensity,
                                                            insert_data))
        analyte_list.append([replicate, int(analyte_data["analyte_id"].max())])
        rt_list.append(analyte_data[analyte_data["scan"] == max_scan]["rt"].max())

    return replicate_max_mass_data, analyte_list, rt_list


def align_replicate_mass_peaks(replicate_max_mass_data):
    """Align mass peaks between replicates
    replicate_mass_peak_list is [[data for each peak as [mz, intensity, relative percentage, replicate number]]]
    """
    ...
    return replicate_mass_peak_list


def average_replicate_peak_intensities(input_structure, replicate_mass_peak_list):
    """Calculate averaged relative intensities across all replicates for each MS peak"""
    ...
    return replicate_mass_peak_data
...
def extract_replicate_ms_data(input_structure, replicate_analyte_data, sample_name):
    """Process replicate MS data and extract required information for replicate analyte objects"""

    replicate_max_mass_data, \
    analyte_list, rt_list = extract_max_intensity_spectra(replicate_analyte_data, sample_name)
    # Align peaks between replicate spectra
    if len(replicate_max_mass_data) > 0:
        replicate_mass_peak_list = align_replicate_mass_peaks(replicate_max_mass_data)
        # If experiment has replicates, calculate average intensity of mass peaks for masses that appear at least twice.
        replicate_mass_peak_data = average_replicate_peak_intensities(input_structure, replicate_mass_peak_list)
        # Check that the max peak is 100% and, if not, fix.
        replicate_mass_peak_data, \
        max_intensity_mass, max_intensity = correct_max_relative_intensity(replicate_mass_peak_data)
        replicate_analyte_rt = round(sum(rt_list)/len(rt_list), 2)

        return replicate_mass_peak_data, analyte_list, replicate_analyte_rt, max_intensity_mass, max_intensity
