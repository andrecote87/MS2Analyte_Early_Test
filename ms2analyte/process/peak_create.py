...
def mass_bin(input_data):
    ...
    for index, row in sorted_input_data.iterrows():
        ...
        if mass_difference > config.mass_error_da:
            peak_counter += 1
        peak_id_list.append(peak_counter)
        previous_mass = row["mz"]

    sorted_input_data["peak_id"] = peak_id_list
...
def mass_range_check(input_data):
    ...
    """
    next_peak_id = int(input_data["peak_id"].max() + 1)
    split_counter = 1
        ...
        for peak, data in input_data.groupby("peak_id"):
            ...
            if peak_max_mass - peak_min_mass > 2 * config.mass_error_da:
                ...
                if len(below_minimum_masses) > 0:
                    input_data.loc[below_minimum_masses, "peak_id"] = next_peak_id
                    next_peak_id += 1
                    split_counter += 1

                if len(above_maximum_masses) > 0:
                    input_data.loc[above_maximum_masses, "peak_id"] = next_peak_id
                    next_peak_id += 1
                    split_counter += 1
...
def flat_peak_slope(peak_data):
    """Calculates slope of peak data points. Used in rolling function to identify flat areas of peaks and remove"""

    return linregress(np.array([1, 2, 3, 4, 5]), peak_data).slope


def assign_high_slope(slope_data):
    """Define a data point as in a high slope region or not, depending on whether following data points are also high
    slope"""

    if (slope_data.abs() < 10).any():
        return False
    # if np.all(np.absolute(~np.isnan(slope_data)) >= 33):
    #     return True
    else:
        return True


def strip_flat_peaks(input_data):
    """Reviews all peaks and identifies regions within the peak that have low slope change and removes them.
    Designed to remove both chemical interference that causes the same mass with a consistent intensity across large
    regions of the run, as well as removing long flat leader or tail regions to 'real' peaks

    """

    # 1. Determine the baseline intensity value of the sample MS data by taking the average of the lowest 20% of the
    # signals by intensity in the raw input file
    # 2. For each data point in the input data, calculate the percentage difference between the intensity and the
    # baseline intensity
    # 3. Calculate the slope of the line of best fit for each percentage difference against the following four
    # percentage differences (five in total)
    # 4. If the absolute value of the slope is greater than 33, mark as 'high slope'
    # 5. review each scan in order. If three or more consecutive scans are low slope, delete them from the peak
    # Prevent this step if the intensity is above some threshold to stop large real but broad peaks from being broken up
    # 6. rerun scan_split, remove_small_peaks, remove_no_maxima, peak_minima_trim

    baseline_intensity = input_data.nsmallest(int(len(input_data.index)/5), ['intensity'])['intensity'].mean()
    print("Baseline intensity = " + str(int(baseline_intensity)))
    input_data['pcent_int_diff'] = ((input_data['intensity'] - baseline_intensity) / baseline_intensity) * 100
    input_data['slope'] = np.nan
    input_data['high_slope'] = np.nan

    for peak, data in input_data.sort_values(['peak_id', 'scan']).groupby('peak_id'):
        if len(data.index) >= 20:
            # Copy data df to remove SettingWithCopyWarning
            temp_data = data.copy()
            # Calculate slope for each data point from rolling set of 5 points. Skips first and last two points
            ...
            if len(temp_data.index) < 200 \
                    ...
                    and temp_data[temp_data.high_slope == 1]["intensity"].min() < 10 * baseline_intensity:
                temp_data.loc[temp_data.intensity > 10 * baseline_intensity, "high_slope"] = 1
            # Remove all 'low slope' data points.
            input_data = input_data.drop(temp_data.loc[temp_data.high_slope == 0].index)

    input_data = input_data.drop(columns=["pcent_int_diff", "slope", "high_slope"])
    input_data = remove_small_peaks(input_data)
    input_data = remove_no_maxima_peaks(input_data)
